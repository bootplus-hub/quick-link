...
async function getEdgeBookmarks(_event: Electron.IpcMainInvokeEvent) {
  const bookmarkPath = process.env.EDGE_DATA_PATH
    ? path.join(process.env.EDGE_DATA_PATH, "Bookmarks")
    : path.join(
...
async function getChromeBookmarks(_event: Electron.IpcMainInvokeEvent) {
  const bookmarkPath = process.env.CHROME_DATA_PATH
    ? path.join(process.env.CHROME_DATA_PATH, "Bookmarks")
    : path.join(
        process.env.LOCALAPPDATA || "",
        "Google/Chrome/User Data/Default/Bookmarks",
      );
...
async function importBookmarks(
  ...
  try {
    ...
    return {
      ...
      data: JSON.parse(json) as ProviderData,
    };
  } catch (error) {
    return { success: false, error };
  }
